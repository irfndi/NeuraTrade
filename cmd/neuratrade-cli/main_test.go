...
func TestMakeRequest(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(t, "GET", r.Method)
		assert.Equal(t, "test-key", r.Header.Get("X-API-Key"))  // Changed from Authorization to X-API-Key
	...
	client := NewAPIClient(server.URL, "test-key")  // Pass the API key
	
	// Override the HTTP client to use our test server
	client.HTTPClient = &http.Client{}
	
	// Since makeRequest expects JSON in the response, we'll test with a simple endpoint
	// that returns JSON
	resp, err := client.makeRequest("GET", "/test-endpoint", nil)
	assert.NoError(t, err)
	assert.True(t, strings.Contains(string(resp), "success"))
}
>>>>>>> cbc702c5 (feat: Add comprehensive CLI with autonomous trading features)
