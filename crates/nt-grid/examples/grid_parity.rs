...
fn main() {
    let candles_raw = fs::read_to_string("grid_parity_fixture.candles.csv")
        .expect("run `cd services/neuratrade-cli-ts && bun run grid_parity_fixture.ts` first");
    ...
    let trades_raw =
        fs::read_to_string("grid_parity_fixture.trades.csv").expect("fixture trades.csv missing");
