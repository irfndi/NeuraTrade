...
module.exports = {
  apps: [
    ...
    {
      name: "neuratrade-btc-candidate-5m",
      script: "bun",
      args: [
        "run",
        "index.ts",
      ...
      max_restarts: 10,
      restart_delay: 30_000,
      out_file: path.join(neuratradeHome, "logs", "btc-candidate-5m.out.log"),
      error_file: path.join(neuratradeHome, "logs", "btc-candidate-5m.err.log"),
      merge_logs: true,
      time: true,
    },
    // STOPPED BY DEFAULT — start deliberately (pm2 start ecosystem.demo-soak.config.cjs
    // --only neuratrade-sol-candidate-5m). The 5m grid configs are exploratory, not
    // gate-validated: same manifest as neuratrade-sol-candidate but at 5m cadence
    // (--interval 300, --timeframe 5m). Logs to sol-candidate-5m.* to avoid clobbering
    // the 15m candidate logs.
    {
      name: "neuratrade-sol-candidate-5m",
      script: "bun",
      args: [
        "run",
        "index.ts",
      ...
      max_restarts: 10,
      restart_delay: 30_000,
      out_file: path.join(neuratradeHome, "logs", "sol-candidate-5m.out.log"),
      error_file: path.join(neuratradeHome, "logs", "sol-candidate-5m.err.log"),
      merge_logs: true,
      time: true,
    },
    {
