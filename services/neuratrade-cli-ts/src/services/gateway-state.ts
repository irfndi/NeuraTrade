...
export interface GatewayStateService {
  ...
  readonly writeMode: (mode: string, detail?: string) => Effect.Effect<void, never>;
...
export const GatewayState = Context.GenericTag<GatewayStateService>("GatewayState");
...
function decodeOrFallback(raw: unknown): Effect.Effect<MutableGatewayState, never> {
  if (raw === null || raw === undefined) return Effect.succeed(defaultGatewayState());
 ...
 */
export const GatewayStateLive: Layer.Layer<GatewayStateService, never, Path | FileSystem.FileSystem> =
  Layer.effect(
    GatewayState,
    Effect.gen(function* () {
      ...
      const writeMode = (mode: string, detail?: string): Effect.Effect<void, never> =>
        Effect.gen(function* () {
      ...
      const markStopped = (detail?: string): Effect.Effect<void, never> =>
        Effect.gen(function* () {
          ...
          mutable.services["gateway"] = { status: "down", detail: effectiveDetail };
      ...
      return { read, write, writeMode, writeServiceState, markStopped } as GatewayStateService;
    }),
  );
