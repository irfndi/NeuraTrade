...
const StubExchangeAdapter = Layer.succeed(ExchangeAdapter, {
  placeOrder: () => Effect.die(new Error("not reached in test")),
  getBalance: () => Effect.die(new Error("not reached in test")),
  getPosition: () => Effect.die(new Error("not reached in test")),
  closePosition: () => Effect.die(new Error("not reached in test")),
});

const StubMarketDataGateway = Layer.succeed(MarketDataGateway, {
  fetchTick: () => Effect.die(new Error("not reached in test")),
