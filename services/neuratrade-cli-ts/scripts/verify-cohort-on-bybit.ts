...
const candidates = [
  { symbol: "BTC/USDT:USDT", label: "BTC step0.5 g3 p48 t2 adx15", g: { gridStepPct: 0.5, gridMaxGrids: 3, gridPauseAfterLossBars: 48, targetRatio: 2, chopGateAdxThreshold: 15 } },
  { symbol: "SOL/USDT:USDT", label: "SOL step1.25 g2 p36 t4 adx26", g: { gridStepPct: 1.25, gridMaxGrids: 2, gridPauseAfterLossBars: 36, targetRatio: 4, chopGateAdxThreshold: 26 } },
  { symbol: "ETH/USDT:USDT", label: "ETH step0.75 g2 p48 t4 adx28", g: { gridStepPct: 0.75, gridMaxGrids: 2, gridPauseAfterLossBars: 48, targetRatio: 4, chopGateAdxThreshold: 28 } },
];
...
for (const c of candidates) {
  const rows = db
    ...
    .all(c.symbol) as Array<{ open: number; high: number; low: number; close: number; volume: number; ts: string }>;
  const candles = rows.map((r) => ({
    open: r.open, high: r.high, low: r.low, close: r.close, volume: r.volume, timestamp: new Date(r.ts),
  }));
  ...
  const r = validateGridEvidence(candles, {
    ...
    grid: {
      ...c.g,
      feePct: 0.02, slippageBps: 1, initialCapital: 100, trendFilterPeriod: 0,
      leverage: 1, positionFraction: 0.5, onlyWithTrend: false,
    },
  ...
  const pass =
    h.profitableWindowPct >= 50 && h.compoundedReturnPct > 0 &&
    h.maximumDrawdownPct <= 15 && r.fixedOos.totalTrades >= 30 &&
    g.lowerBoundPct >= 0 && s.worstReturnPct >= 0 &&
    s.pooledLowerBoundPct >= 0 && h.windows.length >= 10;
  console.log(
