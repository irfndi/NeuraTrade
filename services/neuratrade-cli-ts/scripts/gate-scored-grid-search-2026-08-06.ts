...
const slippage = Number(process.argv[process.argv.indexOf("--slippage-bps") + 1] ?? "2");
const fast = process.argv.includes("--fast");
const home = process.env.NEURATRADE_HOME ?? join(process.env.HOME!, ".neuratrade");
...
interface SweepResult {
  config: { gridStepPct: number; gridMaxGrids: number; gridPauseAfterLossBars: number; targetRatio: number; chopGateAdx: number };
  gates: {
    profitableWindowPct: number; compoundedReturnPct: number; maxDrawdownPct: number;
    fixedOosTrades: number; confidenceLowerBoundPct: number;
    stressWorstReturnPct: number; stressWorstLowerBoundPct: number;
  };
  pass: boolean;
  failures: string[];
}
...
for (const step of steps) for (const maxGrids of grids) for (const pause of pauses)
for (const target of targets) for (const adx of adxGates) {
  total += 1;
  const grid: GridOptions = { ...base, gridStepPct: step, gridMaxGrids: maxGrids, gridPauseAfterLossBars: pause, targetRatio: target, chopGateAdxThreshold: adx };
  const r = validateGridEvidence(candles, { now, grid, executionParityPassed: true });
  if (r.kind === "invalid") {
  ...
  const gate = {
    ...
    stressWorstLowerBoundPct: r.stress.worstLowerBoundPct,
  };
  const fails: string[] = [];
  ...
  if (gate.stressWorstLowerBoundPct < 0) fails.push("stressLB<0");
  results.push({ config: { gridStepPct: step, gridMaxGrids: maxGrids, gridPauseAfterLossBars: pause, targetRatio: target, chopGateAdx: adx }, gates: gate, pass: fails.length === 0, failures: fails });
  if (total % 50 === 0) {
    const elapsedMin = (Date.now() - t0) / 60000;
    console.log(`...${total}/${steps.length * grids.length * pauses.length * targets.length * adxGates.length} after ${elapsedMin.toFixed(1)}m (${(elapsedMin / total * 60).toFixed(1)}s/config, ETA ${(elapsedMin / total * (1728 - total)).toFixed(0)}m) | pass so far: ${results.filter((x) => x.pass).length}`);
  }
}

const passing = results.filter((r) => r.pass).sort((a, b) => b.gates.confidenceLowerBoundPct - a.gates.confidenceLowerBoundPct);
const output = { run: "2026-08-06", fee, slippage, fast, candles: candles.length, total, failures, passing, all: results };
const outPath = join(home, "tuning", `gate-scored-search-2026-08-06-fee${fee}-slip${slippage}.json`);
mkdirSync(dirname(outPath), { recursive: true });
writeFileSync(outPath, JSON.stringify(output, null, 2));
console.log(`\n=== fee ${fee} slip ${slippage}: ${total} configs, ${passing.length} PASS ===`);
for (const p of passing.slice(0, 15)) {
  console.log(`PASS step=${p.config.gridStepPct} grids=${p.config.gridMaxGrids} pause=${p.config.gridPauseAfterLossBars} target=${p.config.targetRatio} adx=${p.config.chopGateAdx} | windows=${p.gates.profitableWindowPct.toFixed(1)}% ret=${p.gates.compoundedReturnPct.toFixed(2)}% dd=${p.gates.maxDrawdownPct.toFixed(2)}% oos=${p.gates.fixedOosTrades} confLB=${p.gates.confidenceLowerBoundPct.toFixed(5)} stressRet=${p.gates.stressWorstReturnPct.toFixed(2)}% stressLB=${p.gates.stressWorstLowerBoundPct.toFixed(5)}`);
}
console.log(`\nresults: ${outPath}`);
