...
const rows = db
  ...
  .all("bitget-futures", "ETH/USDT:USDT", "15m") as {
  open: number;
  high: number;
  low: number;
  close: number;
...
for (const step of [0.3, 0.5, 0.7, 1.0, 1.5]) {
  for (const maxGrids of [2, 3]) {
    ...
    console.log(
      `step=${step}% maxGrids=${maxGrids} -> return=${result.totalReturnPct.toFixed(2)}% trades=${result.totalTrades} win=${result.winRate.toFixed(1)}% dd=${result.maxDrawdownPct.toFixed(2)}% pf=${result.profitFactor.toFixed(3)}`,
    );
  }
}
