...
func (b *BootstrapCommand) Run() error {
	...
	config, err := b.collectConfiguration()
	if err != nil {
...
func (b *BootstrapCommand) collectConfiguration() (*Config, error) {
	config := &Config{}
	...
	defaultJWTSecret, err := generateRandomSecret()
	if err != nil {
		return nil, fmt.Errorf("generate default JWT secret: %w", err)
	}
	fmt.Print("JWT Secret (leave blank for auto-generated): ")
	for {
		config.JWTSecret = b.readInput(defaultJWTSecret)
		if strings.TrimSpace(config.JWTSecret) != "" {
			break
		}
		fmt.Println("JWT Secret cannot be empty.")
		fmt.Print("JWT Secret: ")
		defaultJWTSecret = ""
	}
...
func (b *BootstrapCommand) generateEnvFile(path string, config *Config) error {
	...
	ccxtAdminAPIKey, err := generateRandomSecret()
	if err != nil {
		return fmt.Errorf("generate CCXT admin API key: %w", err)
	}
...
`, timeNow(), config.Environment, config.DatabaseHost, config.DatabasePort,
		...
		ccxtAdminAPIKey, config.TelegramBotToken, config.SentryDSN, config.Environment,
