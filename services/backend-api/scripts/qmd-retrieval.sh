...
DEFAULT_DOCS_PATH="$(cd "$SCRIPT_DIR/../../../docs" && pwd 2>/dev/null)" || DEFAULT_DOCS_PATH=""
...
resolve_docs_path() {
  if [[ -z "$DOCS_PATH" ]]; then
    log_error "DOCS_PATH not set and default path does not exist"
    return 1
  fi
  if [[ ! -d "$DOCS_PATH" ]]; then
    log_error "DOCS_PATH does not exist: $DOCS_PATH"
    return 1
  fi
  return 0
}

find_markdown_docs() { find "$DOCS_PATH" -type f -name "*.md" 2>/dev/null; }
count_docs() { find_markdown_docs | grep -c . 2>/dev/null || echo "0"; }
...
search_with_grep() {
  ...
  if command -v rg &>/dev/null; then
    rg -n --context="$CONTEXT_LINES" --max-count="$MAX_RESULTS" --glob='*.md' "$query" "$DOCS_PATH" 2>/dev/null || true
  else
    grep -rn --context="$CONTEXT_LINES" --max-count="$MAX_RESULTS" --include='*.md' "$query" "$DOCS_PATH" 2>/dev/null || true
  fi
...
index_docs() {
  ...
  local exit_code=0
  if ! qmd create neuratrade --path "$DOCS_PATH" 2>/dev/null; then
    if ! qmd add neuratrade "$DOCS_PATH" 2>/dev/null; then
      log_error "Failed to create or update QMD index"
      exit_code=1
    fi
  fi
  [[ $exit_code -eq 0 ]] && log_success "Indexing complete"
  return $exit_code
}
...
run_benchmark() {
  ...
  for query in "${test_queries[@]}"; do
    ...
    start_time=$(python3 -c 'import time; print(int(time.time() * 1000))' 2>/dev/null) || start_time=$(date +%s000)
    if command -v rg &>/dev/null; then
      result=$(rg -n --context="$CONTEXT_LINES" --max-count="$MAX_RESULTS" --glob='*.md' "$query" "$DOCS_PATH" 2>/dev/null || true)
    else
      result=$(grep -rn --context="$CONTEXT_LINES" --max-count="$MAX_RESULTS" --include='*.md' "$query" "$DOCS_PATH" 2>/dev/null || true)
    fi
    end_time=$(python3 -c 'import time; print(int(time.time() * 1000))' 2>/dev/null) || end_time=$(date +%s000)
...
show_status() {
  ...
  echo -n "grep: "
  command -v grep &>/dev/null && echo "ok" || echo "missing"
  echo -n "rg: "
  command -v rg &>/dev/null && echo "ok" || echo "missing"
  echo -n "qmd: "
  command -v qmd &>/dev/null && echo "ok" || echo "missing"
...
main() {
  resolve_docs_path || {
    echo "Run with DOCS_PATH set to a valid directory"
    exit 1
  }
  ...
  case "$command" in
    ...
    help | --help | -h)
      ...
      echo "Commands: search, index, benchmark, status, resources, help"
      ;;
    *)
