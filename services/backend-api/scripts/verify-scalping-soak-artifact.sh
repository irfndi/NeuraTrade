...
jq -e '
  ...
  (.result.report.action_split | type) == "object" and
  (.result.report.action_split | length) > 0 and
  all(.result.report.action_split[]; (tonumber) >= 0 and (tonumber) <= 1) and
  (.result.report.regime_split | type) == "object" and
  (.result.report.regime_split | length) > 0 and
  all(.result.report.regime_split[]; (tonumber) >= 0 and (tonumber) <= 1) and
...
' "$artifact" >/dev/null || fail "artifact is missing required action/regime split ratios, gate/rejection reason, signal-quality, or proof fields"
