...
HOLD_PERIOD_SECONDS="${HOLD_PERIOD_SECONDS:-30}"
...
MAX_HOLD_RATIO_SET="${MAX_HOLD_RATIO+x}"
...
MIN_BASELINE_WIN_RATE_DELTA_SET="${MIN_BASELINE_WIN_RATE_DELTA+x}"
MIN_BASELINE_NET_PNL_DELTA_SET="${MIN_BASELINE_NET_PNL_DELTA+x}"
MIN_BASELINE_AVG_PNL_DELTA_SET="${MIN_BASELINE_AVG_PNL_DELTA+x}"
...
case "$(printf '%s' "$REQUIRE_TRADES" | tr '[:upper:]' '[:lower:]')" in
  false | 0 | no | off)
    ...
    if [ -z "$MIN_BASELINE_NET_PNL_DELTA_SET" ]; then
      MIN_BASELINE_NET_PNL_DELTA=""
    fi
    if [ -z "$MIN_BASELINE_AVG_PNL_DELTA_SET" ]; then
