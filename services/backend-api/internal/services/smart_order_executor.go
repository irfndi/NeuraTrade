...
func (e *SmartOrderExecutor) executeWithRetry(ctx context.Context, req SmartOrderRequest, maxRetries int, startTime time.Time) (*SmartOrderResult, error) {
	...
	var totalFillPrice decimal.Decimal

	for attempt := 1; attempt <= maxRetries; attempt++ {
		orderAmount := req.Amount.Sub(totalFilled)
		if orderAmount.LessThanOrEqual(decimal.Zero) {
			return &SmartOrderResult{
				...
				RemainingAmount: decimal.Zero,
				FillPrice:       totalFillPrice,
				SlippagePercent: e.calcSlippage(req, totalFillPrice),
				Attempts:        attempt - 1,
				ExecutionTime:   time.Since(startTime),
			}, nil
		}
		...
		orderID, err := e.config.BaseExecutor.PlaceOrder(ctx, req.Exchange, req.Symbol, req.Side, orderType, orderAmount, req.Price)
		if err != nil {
			lastErr = err
			lastOrderID = orderID
		...
		if req.OrderType == OrderTypeMarket {
			...
			if fillErr != nil {
				if filled.GreaterThanOrEqual(req.Amount.Sub(totalFilled)) {
					...
					return &SmartOrderResult{
						...
						Status:          "filled",
						FilledAmount:    totalFilled,
						RemainingAmount: req.Amount.Sub(totalFilled),
						FillPrice:       totalFillPrice,
...
func (e *SmartOrderExecutor) waitForFill(ctx context.Context, req SmartOrderRequest, orderID string, timeout time.Duration) (decimal.Decimal, decimal.Decimal, error) {
	...
	for {
		...
		case <-ticker.C:
			...
			if !foundInOpen {
				return decimal.Zero, decimal.Zero, errors.New("order not found in open or closed orders")
			}
		}
	}
}
