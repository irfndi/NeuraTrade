...
func TestAnalystAgent_Metrics(t *testing.T) {
	...
	_ = agent.Analyze(context.Background(), "BTC/USDT", AnalystRoleTechnical, bullishSignals)
	_ = agent.Analyze(context.Background(), "ETH/USDT", AnalystRoleSentiment, bearishSignals)
	_ = agent.Analyze(context.Background(), "SOL/USDT", AnalystRoleTechnical, neutralSignals)
