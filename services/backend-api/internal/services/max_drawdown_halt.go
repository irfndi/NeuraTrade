...
func (h *MaxDrawdownHalt) notifyRiskEvent(state *DrawdownState, eventType, severity string) {
	...
	// Send asynchronously so state processing is not blocked by downstream network I/O.
	go func(chatID int64, evt RiskEventNotification, evtType string) {
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()
		if err := h.notificationSvc.NotifyRiskEvent(ctx, chatID, evt); err != nil {
