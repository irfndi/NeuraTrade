...
func TestExecuteRoutinePaperTradingReviewBlocksUntilEvidenceArtifactExists(t *testing.T) {
	...
	assert.Equal(t, "diagnostic_probe", quest.Checkpoint["paper_trading_readiness_evidence_metrics_status"])
	...
	assert.Equal(t, 0, metrics["continuous_validation_hours"])
	assert.Equal(t, 0, metrics["strategy_count"])
...
func TestExecuteRoutinePaperTradingReviewRecordsLifecycleDiagnostics(t *testing.T) {
	...
	assert.Equal(t, "diagnostic_lifecycle", quest.Checkpoint["paper_trading_readiness_evidence_metrics_status"])
