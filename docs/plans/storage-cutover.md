...
- Recipe (from a green run):
  ...
  `ssh root@$BOX_HOST 'mkdir -p /opt/neuratrade/bin'` (first deploy only;
  dir does not exist yet — verified 2026-09-18)
  ...
  Health gate (ship is NOT done until this passes):
  `/opt/neuratrade/bin/nt-cli health` must print `status=ok` BEFORE any
  `pm2 restart`; after restart, probe again. A bad cross-build never swaps.
