...
native-check: ## Rust fmt + clippy (zero warnings) + all 8 example checks
	...
	@cd crates && cargo run -p nt-grid --example sleeves_check
	@cd crates && cargo run -p nt-grid --example pump_catch
